...
def sigmoid(x):
    return numpy.exp(x - 0.5) / (numpy.exp(x - 0.5) + numpy.exp(0.5 - x))
...
class PlotLossCallback(LossCallback):
    ...
    def __init__(self, report_interval = 100, save = '', show = False):
        ...
        self._input_sizes = manager.list([])
        self._output_sizes = manager.list([])
        self._label_sizes = manager.list([])
        """ ([float]) Takes all losses. """
    ...
    def _get_loss(self, iteration, solver):
        ...
        if len(self._iterations) == 0 or self._iterations[-1] != iteration:
            self._iterations.append(iteration)
            self._losses.append(solver.net.blobs['loss'].data)
            self._input_sizes.append(solver.net.blobs['data'].data.shape)
            self._output_sizes.append(solver.net.blobs['concat2'].data.shape)
            self._label_sizes.append(solver.net.blobs['labels'].data.shape)
    ...
    def report_loss(self, iteration, solver):
        ...
        if iteration%self.report_interval == 0:
            print('Training loss [' + str(self._iterations[-1]) + ']: ' + str(self._losses[-1]) + ' input size : ' + str(self._input_sizes[-1]) + ' output size : ' + str(self._output_sizes[-1]) + ' label size : ' + str(self._label_sizes[-1]))
...
class ErrorCallback(object):
    ...
    def _copy_weights(self, net_from, net_to):
        ...
        print("params length: {}  ,  params: {}".format(len(params), params))
        Flag = False
        for pr in params:
            if net_to.params[pr][1] != net_from.params[pr][1] or net_to.params[pr][0] != net_from.params[pr][0]:
                flag = True
            net_to.params[pr][1] = net_from.params[pr][1]
            net_to.params[pr][0] = net_from.params[pr][0]
        print("Whether the parameters are updated: ", Flag)
    ...
    def _compute_errors(self, type, iteration, net):
        ...
        print("error phase:{}  ,  size:{}".format(type, size))
        if size > 0: # only compute error if size is given
            if len(self._errors[type]) == 0 or self._iterations[type][-1] != iteration:
                ...
                for b in range(number_batches):
                    net.forward()
                    scores = sigmoid(net.blobs['fusion'].data)
                    ...
                    print('Computing errors ...  ' + str(errors) + "  at testing sample  " + str(b) + "  " + str(b/float(number_batches)) + "   input size:" + str(net.blobs['data'].data.shape) + "  output size:" + str(scores.shape) + "   label size:" + str(labels.shape) + '%\r', end = '') # http://stackoverflow.com/questions/4897359/output-to-the-same-line-overwriting-previous-output-python-2-5
